...
#include "bananarino.h"
#include "bananarino_icons.h"
...
const char* about[] = {
	...
	"Banana is life",
	"~Ayylmao~",
	"Edited by Exclavia, 2026",
	"Press any key to return",
	0,
};

const char* resetProg[] = {
	"All progress resetted.",
	"Press any key to return",
	0,
};

const char* cool_text[] = {
	"wow",
	"cool",
	"nice",
	"rad",
...
const char* cool_message(uint32_t index) {
	const char* current_message = cool_text[index];
...
void draw_banana(Canvas* canvas, GameState* state) {
	...
	if(state->dingus && state->counter > 0) {
		char dingus_str[24];
		snprintf(dingus_str, sizeof(dingus_str), "%s (%lu)", cool_message(state->msg_count), state->d_counter);
		canvas_draw_str_aligned(canvas, 64, 62, AlignCenter, AlignBottom, dingus_str);
	} else {
		// Draw counter
		char counter_str[16];
		snprintf(counter_str, sizeof(counter_str), "%lu", state->counter);
		canvas_draw_str_aligned(canvas, 64, 62, AlignCenter, AlignBottom, counter_str);
	}
}
...
void draw_reset(Canvas* canvas) {
	uint8_t size = 0;
	while(resetProg[size] != 0)
		size++;
	...
	int item_max_width = 0;
	for(uint8_t i = 0; i < size; i++) {
		int item_width = canvas_string_width(canvas, resetProg[i]);
		if(item_width > item_max_width) {
			item_max_width = item_width;
		}
	}
...
void draw_callback(Canvas* canvas, void* ctx) {
	...
	} else if(app_state->app_status == RESETTING) {
		draw_reset(canvas);
		reset_state(app_state->game_state);
	}
...
int32_t banana_main(void* p) {
	...
	app_state->game_state->dingus = false;
	app_state->game_state->d_counter = 0;
	app_state->game_state->msg_count = 0;
			...
			if(app_state->app_status == MENU) {
					...
					case InputKeyOk:
						switch(menu[app_state->menu_selected].id) {
							...
							case EXIT:
								...
								break;
							case RESETPROG:
								app_state->app_status = RESETTING;
								break;
							default:
								break;
						}
				...
				switch(event.key) {
					case InputKeyOk:
						...
						if (app_state->game_state->counter > 0) {
							int counter_mod = app_state->game_state->counter % 50;
							if (counter_mod == 0) {
								app_state->game_state->d_counter++;
								app_state->game_state->dingus = true;

								if (app_state->game_state->msg_count > 0 || app_state->game_state->msg_count == 0) {
									if (app_state->game_state->msg_count < 18) {
										app_state->game_state->msg_count++;
									} else {
										app_state->game_state->msg_count = 0;
									}
								}
